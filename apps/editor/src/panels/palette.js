  ...
  editorStore.subscribe(() => {
    ...
    for (const btn of terrainGrid.querySelectorAll(".palette-btn")) {
      btn.classList.toggle(
        "active",
        activeTool === "terrain" && btn.dataset.terrain === selectedTerrain,
      );
    }
    for (const btn of objGrid.querySelectorAll(".palette-btn")) {
      btn.classList.toggle(
        "active",
        activeTool === "object" && btn.dataset.objectType === selectedObjectType,
      );
    }
    for (const btn of wallGrid.querySelectorAll(".palette-btn")) {
      btn.classList.toggle(
        "active",
        activeTool === "wall" && btn.dataset.wallType === selectedWallType,
      );
    }
  });
}
