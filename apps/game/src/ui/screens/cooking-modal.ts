...
const COOKING_RECIPES = Object.values(CRAFT_ACTIVITIES).filter((a) => a.skill === "cooking");

export function renderCookingModal(container: HTMLElement): void {
  const state = gameStore.getState();
  const cookingLevel = state.skills.cooking.level;

  const recipes = COOKING_RECIPES.map((recipe) => {
    const meetsLevel = cookingLevel >= recipe.levelRequired;
  ...
  container.innerHTML = `
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p style="font-size:12px;color:rgba(244,228,193,0.6);margin-bottom:12px">Cooking Level: ${cookingLevel}</p>
